import { homepage_url,trip_creation_url } from "../../constants/constants.js";

// Export user id
let user_id_export = localStorage.getItem("user_id");
console.log(user_id_export);

// Function to fetch user trip details
export async function fetchdetails(user_id_export) {
    try {
...
function storetripId(tripId, inputId) {
    if (inputId === "input-101") {
        ...
        console.log("Start Trip ID saved to localStorage:", tripId);
    } else if (inputId === "input-10") {
        localStorage.setItem('endTripId', tripId);
        console.log("End Trip ID saved to localStorage:", tripId);
    }
}
    ***/
function storetripId(tripId, inputId) {
...
function initializeHomepageFunctions() {
    ...
    // Post trip details
    postDetailsButton.addEventListener('click', async function checktrips() {
        try {
            const response = await fetch(trip_creation_url, {
                method: 'POST',
                headers: {
                    'uid': user_id_export,
                    'Content-Type': 'application/json',
